...
namespace Markdown.Tests
{
    ...
    public class Md_ShouldRender
    {
        public static IEnumerable<TestCaseData> SpecifictaionCases()
        {
            yield return
                new TestCaseData(
                    "Текст _окруженный с двух сторон_  одинарными символами подчерка должен помещаться в em")
                    .Returns(
                        "<p>Текст <em>окруженный с двух сторон</em>  одинарными символами подчерка должен помещаться в em</p>")
                    .SetName("Курсив");
            ...
            yield return new TestCaseData("[внутри ссылок работает _курсив_ и __жирный__](http://ссылка)")
                .Returns(
                    "<p><a src='http://ссылка'>внутри ссылок работает <em>курсив</em> и <strong>жирный</strong></a></p>")
                .SetName("Ссылки с курсивным и жирным выделением");

            yield return
                new TestCaseData("Если между двух строк есть пустая строка\r\n\r\nто эти строки в разных параграфах")
            ...
            yield return
                new TestCaseData("Когда строка заканчивается на два пробела или более  \r\nто происходит перенос")
        ...
        [TestCase("a\r\n   \r\nb", ExpectedResult = "<p>a</p><p>b</p>",
            TestName = "Строка считается пустой если она пробельная")]
        [TestCase("a\r\n\r\n\r\nb", ExpectedResult = "<p>a</p><p>b</p>",
            TestName = "Параграфы могут быть отделены несколькими пустыми строками")]
